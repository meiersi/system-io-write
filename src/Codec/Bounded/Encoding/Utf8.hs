...
--
module Codec.Bounded.Encoding.Utf8
    ( 
      -- * Characters
      char
    ...
    , wordHex
      ...
      --
      -- You can encode other primitive Haskell values by converting them to
      -- the equally-sized 'Word' type and using the corresponding fixed-width
      -- hexadecimal encoding. For example, you can encode 'Int32' values using
      -- a fixed-width hexadecimal encoding as follows.
      --
      -- > showEncoding (word32HexFixed #. fromIntegral) (-1 :: Int32) = "ffffffff" 
      --
      -- TODO: Comment on the conversion to use for 'Float' and 'Double' values.
      --
    , word8HexFixed
    , word16HexFixed
    , word32HexFixed
    ...
    -- , doubleHexFixed
    --
    -- because they are not simple. They are just the compositions
    --
    -- word32Hex #. coerceFloatToWord32
    -- word64Hex #. coerceDoubleToWord64
    -- word32HexFixed #. coerceFloatToWord32
    -- word64HexFixed #. coerceDoubleToWord64
    --
    -}
    ) where
